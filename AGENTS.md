...
- Keep Actions permissions read-only. Never restore force pushes, direct branch writes, downstream dispatches, automatic publication or personal-token dependencies.
- `.upstream.json` is explicit provenance, never infer it from commit parents. Stop on conflicts or missing provenance.
